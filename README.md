...
- ✍️ **文字转语音**：输入文字（≤1000 字），选声音角色，一键合成 wav，网页内直接试听
- 🗣️ **多音色管理**：模型目录扫描自动识别；支持删除角色、缺文件提示；首次加载后常驻缓存秒回
- 🚀 **缓存策略可选**：默认「单角色缓存」（切换音色自动释放上一个，省显存）；页面勾选可开「多角色缓存」
...
- ✂️ **长文本稳合成**：自动清洗 markdown/URL/特殊符号，20 字阈值分句 + 交叉淡化拼接，减少漏字跳读
