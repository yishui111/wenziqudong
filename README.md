...
- **稳定工程化**：防重复双击、单/多角色缓存策略、文本自动清洗与分句拼接，长文合成也稳
...
- 🧰 **网页内服务维护**：一键「释放显存」、「⏹ 关闭服务」（彻底关闭进程，绝不自动重启），不用敲命令
- 🔌 **OpenAI 兼容接口**：`/v1/audio/speech`（mp3）、`/v1/audio/voices`、`/v1/models`，供 Open WebUI 等外部系统直接调用
- 🛡️ **启停可控**：服务在前台黑框窗口运行、日志实时滚动；**启动与否完全由你决定**——双击 start.bat 才启动，关闭（stop.bat / 关窗口 / 网页按钮）就是彻底关闭，**绝无自动启动、绝不自动重启**；**关闭窗口 = 服务彻底结束，立即释放显卡与内存**（Windows Job 对象保证，不留孤儿进程）；一键启动防重复双击
...
│   ├── tts_run.ps1           # 启动窗口：把服务拉起一次（绝不自动重启）
...
> 首次启动需加载 torch / 预训练模型，约 **5–10 分钟**就绪（服务日志在 `tts_service\tmp\`）；就绪后脚本会自动打开浏览器。
...
| `/api/shutdown` | POST | 彻底关闭服务（退出进程，**绝不自动重启**；再次使用需重新运行 start.bat） |
