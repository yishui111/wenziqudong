...
- 看门狗后台运行，python 异常退出 **3 秒自动重启**（连续 3 次 30 秒内快速退出则看门狗自停，防死循环）。
...
- **双击没反应 / 服务起不来**：先查 `tts_service\tmp\tts_python.log.err` 与 `tts_watchdog.log`。常见：`gptsovits\GPT-SoVITS` 或 `runtime\py312\python.exe` 不存在、8060 被占用、没放音色模型（会直接报「没有可用的声音模型」）。
